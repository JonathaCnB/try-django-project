...
class ArticleTestCase(TestCase):
    def setUp(self):
        ...
        for i in range(0, self.number_of_articles):
            Article.objects.create(title="tesss", content="something else")
    ...
    def test_hello_word_unique_slug(self):
        qs = Article.objects.exclude(slug__iexact="tesss")
        for obj in qs:
            title = obj.title
            slug = obj.slug
            slugified_title = slugify(title)
    ...
    def test_article_search_manager(self):
        qs = Article.objects.search(query="tesss")
        self.assertEqual(qs.count(), self.number_of_articles)
        qs = Article.objects.search(query="some")
        self.assertEqual(qs.count(), self.number_of_articles)
        qs = Article.objects.search(query="else")
        self.assertEqual(qs.count(), self.number_of_articles)
